...
try:
    ...
    try:
        ...
        from PyQt5.QtGui import QPolygon, QPolygonF, QColor, QPen, QFont, QBrush
        from PyQt5.QtGui import QPainter, QFontMetrics, QConicalGradient, QLinearGradient
...
class AnalogGauge(QWidget):
    ...
    def __init__(self, parent=None, typeOf='Bar'):

        super(AnalogGauge, self).__init__(parent)
    ...
    def draw_filled_polygon(self, outline_pen_with=0):
        if self.type == 'Bar':
            ...
            _grad1 = QLinearGradient(int(w * 0.5), 0, int(w * 0.5), h)
            _grad1.setColorAt(0.1, Qt.red)
            _grad1.setColorAt(0.5, Qt.yellow)
            _grad1.setColorAt(0.6, Qt.green)

            painter.setPen(Qt.NoPen)
            painter.setBrush(QBrush(_grad1))

            painter.drawRoundedRect(int(w / 4 * 3), 0, int(w), h, 1.8, 1.8)
    ...
    def draw_big_scaled_markter(self):
        if self.type == 'Bar':
            ...
            # steps_size = (self.value_max-self.value_min) / self.scala_main_count
            steps_size = (self.height() - 10) / self.scala_main_count

            for i in range(1, (self.scala_main_count) + 2):
                my_painter.drawLine(w / 2, steps_size * i, ((w / 4) * 3) * 0.97, steps_size * i)
    ...
    def create_scale_marker_values_text(self):
        ...
        if self.type == 'Bar':
            ...
            # scale_per_div = int(self.height() / self.scala_main_count)
            steps_size = (self.height() - 10) / self.scala_main_count

            for i in range(1, self.scala_main_count + 2):
                text = str(int(self.value_max - (self.value_min + scale_per_div * i)))
                painter.setFont(font)
                painter.drawText((self.width() / 4) * 0.7, steps_size * i, text)
    ...
    def create_fine_scaled_marker(self):
        if self.type == 'Bar':
            ...
            # steps_size = (self.value_max-self.value_min) / (self.scala_main_count * self.scala_subdiv_count)
            steps_size = (self.height() - 10) / (self.scala_main_count * self.scala_subdiv_count)

            for i in range(5, (self.scala_main_count * self.scala_subdiv_count) + 1):
                my_painter.drawLine(w / 2 * 1.2, steps_size * i, ((w / 4) * 3) * 0.97, steps_size * i)
    ...
    def draw_needle(self):
        if self.type == 'Bar':
            ...
            painter.translate(0, self.height() / 3)
            painter.setPen(Qt.NoPen)
